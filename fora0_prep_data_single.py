...
for itr, fle in enumerate(glob.glob(path + "raw_data/test_data.*.blck01.csv")):
    ...
    dt['foraging T/F'] = dt['foraging T/F'].astype('str').map({'True': True, "TRUE": True, 'False': False, "FALSE": False, "None": False}).astype(bool)
    ...
    for its, row in dtC.iterrows():
        ...
        if row['foraging T/F'] == "True":
            resp.append(1)
        elif row['foraging T/F'] == True:
            resp.append(1)
        elif row['foraging T/F'] == "False":
            resp.append(0)
        elif row['foraging T/F'] == False:
            resp.append(0)
    ...
    if np.all(resp) == False and len(resp)/8 < len(resp) - np.count_nonzero(resp == 1) < len(resp) - len(resp)/8:
        ...
        model = np.array(dtC['p_succ_uncorrected']*dtC['gain_if_succ'] + (1-dtC['p_succ_uncorrected'])*-2 + dtC['r_threat']*-3)
        # Add model to datasheet
