...
# Select multivariate model if True
multi = True
...
if multi == False:
    data = pd.read_csv(path + "DATA_clean/DATA_group_level/test_data.group_level_univar.csv")
else:
...
data['bins'] = pd.qcut(data['optimal policy values'],11)
data['binNr'] = [(
...
# Filtering
data = data[data['ternary state'] == 2]
data = data[data['binNr'] < 0.33]
data = data[data['binNr'] >-0.33]
...
# Aggregate
top = data.groupby(['binNr','participant'])[mdls].mean()
...
plt.xlabel("$\\mathit{OP}$ values + cap", fontsize=30)
