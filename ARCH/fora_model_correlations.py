...
""" Correlation matrix and heatmap for all models """
...
data[r'$\Delta Q$-values'] = data['optimal policy values']
del data['optimal policy values']
data['multi-feature policy'] = data['multi-heuristic policy']
del data['multi-heuristic policy']
data['expected gain'] = data['expected gain naive']
del data['expected gain naive']
# Revise list of model names
mdlName =   ['win stay lose shift',
            'wait when safe',
            'binary energy state',
            'weather type',
...
if fltr == True:
    # Data fltring
    data = data[data['multi-feature policy'] != 1]
    data = data[data['multi-feature policy'] != 0]
    # Revise list of model names
    mdlName =   ['win stay lose shift',
                # 'wait when safe',
                # 'binary energy state',
