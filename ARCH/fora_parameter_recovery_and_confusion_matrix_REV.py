...
# %%
"""=== Run model recovery simulations ==="""
...
os.chdir(path)
import fora_logit_coefficients_REV
mdlName, mu_b0, mu_b1, si_b0, si_b1, b0_subs, b1_subs, cof_all, min_b0, max_b0, min_b1, max_b1 = fora_logit_coefficients_REV.get_BIC()
...
for itr, mod in enumerate(mdlName):
    ...
    for epi in range(epis):
        ...
        b0 = np.clip(
            np.random.normal(mu_b0[itr], si_b0[itr]),
            min_b0[itr],
            max_b0[itr]
        )
        b1 = np.clip(
            np.random.normal(mu_b1[itr], si_b1[itr]),
            min_b1[itr],
            max_b1[itr]
        )
            ...
            for day in range(nDay): 
                ...
                if lp_cur == 0:
                    ...
                    if np.isnan(x) == False:
                        ...
                        # Run activation function with lpse noise
                        p_fora = (1 - lapse) * activate(dv) + lapse * 0.5
        ...
        for jtr, mad in enumerate(mdlName):
            mods_j = np.array(mods[jtr])
            resp_j = np.array(resp)

            valid = ~np.isnan(mods_j)

            x_true = sm.add_constant(mods_j[valid])
            y_true = resp_j[valid]

            model = sm.Logit(y_true, x_true)
            fitti = model.fit(disp=0)

            BIC.append(fitti.bic)

            # Store params of the TRUE generating model
            if mad == mod:
                true_params = fitti.params
        
        # Determin winning model
        best_model = np.argmin(BIC)
        winr[best_model] += 1

        # If the winning model is the true model, store the parameters for parameter recovery
        if mdlName[best_model] == mod and true_params is not None:
            modell_b0.append(true_params[0])
            modell_b1.append(true_params[1])
            choice_b0.append(b0)
            choice_b1.append(b1)
    ...
    beta1 = pd.DataFrame(list(zip(modell_b1, choice_b1)), columns =  ["Beta1 model", "Beta1 target"])
    # Correlation beta0
    if len(modell_b0) > 5:
        r, p = pearsonr(modell_b0, choice_b0)
    ...
    # Correlation beta1
    if len(modell_b1) > 5:
        r, p = pearsonr(modell_b1, choice_b1)
    sns_plot = sns.lmplot(data = beta1, x="Beta1 model", y="Beta1 target")
    plt.gcf().set_size_inches(6, 6)
    plt.tick_params(axis="x", labelsize=24)
    plt.tick_params(axis="y", labelsize=24)
    plt.title(mod, fontsize = 38)
