...
# Plots for multivariate analysis (if true)
multi = False
...
if multi == True:
    ## Get data and names
    if condition == 1:
        peps = pd.read_csv(path + "/RESULTS/fora_PEPs_approach.csv", header=None)
    elif condition == 2:
        peps = pd.read_csv(path + "/RESULTS/fora_PEPs_avoidance.csv", header=None)
    else:
    ...
    peps = pd.read_csv(path + "/RESULTS/fora_PEPs_univariate.csv", header=None)
    valu = peps.values.tolist()[0]
    name = [
        '#12',
        '#11',
